...
Page {
    ...
    function fnReadSSHFile(strFilePath: string) {
        ...
        if (iRet === -1) {
            ...
            if (iRet === 2) {
                ...
                componentDialog = Qt.createComponent("qrc:///ExtendQML/WarningDialog.qml");
                if (componentDialog.status === Component.Ready) {
                    let warningDialog = componentDialog.createObject(pageSSHServer,
                                                                {viewHeight: pageSSHServer.height * 0.8, bDark: pageSSHServer.bDark,
    ...
    function fnReadSSHFileFromServer(remoteFilePath: string, strContents: string) {
        ...
        if (iRet === -1) {
            ...
            // Save the selected sshd_config file to ini file.
    ...
    header: Column {
        ...
        TabBar {
            ...
            TabButton {
                id: tabGeneral
                text: qsTr("General")
                width: Math.max(300, mainBar.width / mainBar.count)

                font.pointSize: 14 + pageSSHServer.fontPadding
        ...
        RowLayout {
            ...
            Button {
                ...
                background: Rectangle {
                    implicitWidth: pageSSHServer.fontPadding === 0 ? 250 : pageSSHServer.fontPadding === 3 ? 300 : 200
                    implicitHeight: pageSSHServer.fontPadding === 0 ? 60 : pageSSHServer.fontPadding === 3 ? 80 : 50
    ...
    StackLayout {
        id: stackLayout
        x: 20
        width: parent.width - 20// - anchors.leftMargin
        height: pageSSHServer.height - headerSSHServer.height
